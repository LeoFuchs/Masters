...
def string_formulation(model, feature_names, number_words, number_topics, similar_words, levenshtein_distance,
                       pubyear, pubyear2, bert_model, bert_tokenizer):
    ...
    Args:
        ...
        pubyear2: Search year delimiter.
        bert_tokenizer:
        bert_model:
...
def enrichment_words(word, bert_model, bert_tokenizer):
    ...
    read_files = glob.glob(
        "/home/fuchs/Documentos/MESTRADO/Masters/Files-QGS/revisao-%s/QGS-txt/metadata/txt/*.txt" % author)
...
def scopus_search(string):
    ...
    search_df[['title']].to_csv("/home/fuchs/Documentos/MESTRADO/Masters/Code"
                                ""
                                "/Exits/Result.csv", index_label=False,
...
def main():
    ...
    pubyear2 = 2005  # Pubyear2 = 0 --> disable
    qgs_size = 45
    gs_size = 138
    ...
    print("Open Snowballing...\n")
    with open('title-list-snowballing.txt', 'rb') as title_list_snow:
        title_list = pickle.load(title_list_snow)

    with open('adjacency-matrix-snowballing.txt', 'rb') as adjacency_matrix_snow:
        adjacency_matrix = pickle.load(adjacency_matrix_snow)

    with open('final-edges-snowballing.txt', 'rb') as final_edges_snow:
        final_edges = pickle.load(final_edges_snow)
                ...
                for number_words in number_words_list:
                    ...
                    for enrichment in enrichment_list:
                        string = string_formulation(lda, dic, number_words, number_topics, enrichment,
                                                    levenshtein_distance, pubyear, pubyear2, bert_model, bert_tokenizer)
