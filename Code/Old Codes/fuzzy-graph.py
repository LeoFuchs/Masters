...
def graph_snowballing(results_list):
   ...
   """
    ...
    with open('/home/fuchs/Documentos/MESTRADO/Masters/Files-QGS/revisao-menezes/GS.csv', mode='r') as gs:
        ...
        title_list = [line.strip().lower().replace(' ', '') for line in gs]
    ...
    for i in range(1, len(title_list) + 1):
        article_name = '/home/fuchs/Documentos/MESTRADO/Masters/Files-QGS/revisao-menezes/GS-pdf/%d.bibtex' % i
        with open('%s' % article_name, mode='r') as file_zone:
            for j in range(1, len(title_list) + 1):
                final_ratio = 1
                if i != j:
                    for line in file_zone:
                        #ratio = fuzz.ratio(line.strip().lower().replace(' ', '').replace('-', '')
                        #                   .replace(':', '').replace('\'', '').replace(',', '')
                        #                   .replace('?', '').replace('s', ''), title_list[j-1])
                        ratio = fuzz.partial_ratio(line.strip().lower().replace(' ',''), title_list[j-1])
...
def  main():
    ...
    cermine = "java -cp cermine-impl-1.14-20180204.213009-17-jar-with-dependencies.jar " \
              ...
              "/home/fuchs/Documentos/MESTRADO/Masters/Files-QGS/revisao-menezes/GS-pdf -outputs bibtex "
    os.system(cermine)
